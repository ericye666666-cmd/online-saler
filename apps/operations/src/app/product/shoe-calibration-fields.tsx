...
export function ShoeCalibrationFields(props: {
  ...
  return (
    <FieldSet>
      <FieldLegend>{t("鞋码与成双检查")}</FieldLegend>
      <FieldDescription>{t("一双鞋对应一个商品和条码。请对照整双、侧面、鞋底及尺码标原图检查。")}</FieldDescription>
      <FieldGroup>
        <Field data-field-key="shoeSizeSystem" data-invalid={!form.shoeSizeSystem} data-disabled={disabled}>
          <FieldLabel htmlFor={`${id}-system`}>{t("鞋码制式 *")}</FieldLabel>
          <NativeSelect id={`${id}-system`} className="w-full" value={form.shoeSizeSystem} disabled={disabled} aria-invalid={!form.shoeSizeSystem} onChange={(event) => onChange("shoeSizeSystem", event.target.value)}>
            <NativeSelectOption value="">{t("请选择鞋标上的码制")}</NativeSelectOption>
            {SHOE_SIZE_SYSTEMS.map((system) => <NativeSelectOption key={system} value={system}>{t(SYSTEM_LABELS[system])}</NativeSelectOption>)}
          </NativeSelect>
        </Field>
        <Field data-field-key="tagSize" data-invalid={!sizeLabel} data-disabled={disabled}>
          <FieldLabel htmlFor={`${id}-size`}>{t("原标鞋码 *")}</FieldLabel>
          <Input id={`${id}-size`} value={form.tagSize} disabled={disabled} maxLength={80} aria-invalid={!sizeLabel} placeholder={t("例如 42、7.5，按实物标签填写")} onChange={(event) => onChange("tagSize", event.target.value)} />
          <FieldDescription>{sizeLabel ? t("商城显示：{sizeLabel}。", { sizeLabel: sizeLabel }) : t("鞋标不清楚时保留草稿，核实后再确认。")}  {t("不按年龄、鞋底长度或服装尺码猜测换算。")}</FieldDescription>
        </Field>
        <Field data-field-key="shoeType" data-invalid={!form.shoeType} data-disabled={disabled}>
          <FieldLabel htmlFor={`${id}-type`}>{t("鞋款 *")}</FieldLabel>
          <NativeSelect id={`${id}-type`} className="w-full" value={form.shoeType} disabled={disabled} aria-invalid={!form.shoeType} onChange={(event) => onChange("shoeType", event.target.value)}>
            <NativeSelectOption value="">{t("请选择鞋款")}</NativeSelectOption>
            {SHOE_TYPES.map((type) => <NativeSelectOption key={type} value={type}>{t(TYPE_LABELS[type])}</NativeSelectOption>)}
          </NativeSelect>
        </Field>
        <Field orientation="horizontal" data-field-key="shoePairConfirmed" data-invalid={!form.shoePairConfirmed} data-disabled={disabled}>
          ...
          <FieldLabel htmlFor={`${id}-pair`}>{t("我已检查：左右鞋同款、同码，完整成双 *")}</FieldLabel>
        </Field>
        <Field data-field-key="shoeConditionNotes" data-invalid={!form.shoeConditionNotes.trim()} data-disabled={disabled}>
          <FieldLabel htmlFor={`${id}-condition`}>{t("鞋况检查 *")}</FieldLabel>
          <Textarea id={`${id}-condition`} rows={3} value={form.shoeConditionNotes} disabled={disabled} aria-invalid={!form.shoeConditionNotes.trim()} onChange={(event) => onChange("shoeConditionNotes", event.target.value)} placeholder={t("例如：鞋底轻微磨损，鞋面无裂纹，内里完整，无开胶。")} />
          <FieldDescription>{t("人工检查鞋底、鞋面、内里和开胶情况，如实填写；有瑕疵须补特写并填写下方瑕疵。")}</FieldDescription>
        </Field>
        <Field data-field-key="insoleLengthCm" data-disabled={disabled}>
          <FieldLabel htmlFor={`${id}-insole`}>{t("鞋垫实测长度（cm，可选）")}</FieldLabel>
          ...
          <FieldDescription>{t("仅在鞋垫可取出且已人工实测时填写。鞋底长度不等于鞋内长度。")}</FieldDescription>
        </Field>
      </FieldGroup>
    </FieldSet>
  );
