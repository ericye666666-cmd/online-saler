...
export function ManualMeasurementEditor(props: {
  ...
  function addPoint(event: ReactPointerEvent<HTMLCanvasElement>) {
    ...
    if (measuredValue === null) {
      setError(t("无法根据测量板计算这条线，请重新校准板面后再试。"));
      setStart(null);
      return;
    }
  ...
  function addBoardCalibrationPoint(point: MeasurementPoint) {
    ...
    if (!validMeasurementBoardCalibration(calibration)) {
      setCalibrationPoints([]);
      setError(t("板面四角无效，请按左上、右上、右下、左下的顺序重新点击。"));
      return;
    }
  ...
  function apply() {
    if (!boardCalibration || calibratingBoard) {
      setError(t("请先完成测量板四角校准。"));
      return;
    }
    const invalid = lines.find((line) => !Number.isFinite(Number(line.valueCm)) || Number(line.valueCm) <= 0);
    if (invalid) {
      setActiveKey(invalid.key);
      setError(t("{v0}需要填写大于 0 的厘米值。", { v0: labels.get(invalid.key) ?? t("该尺寸") }));
      return;
    }
    if (!lines.length) {
      setError(t("请至少选择一个尺寸并在原图上连接起点和终点。"));
      return;
    }
  ...
  return (
    <Dialog open={props.open} onOpenChange={props.onOpenChange}>
      <DialogContent className="max-h-[calc(100vh-1rem)] overflow-y-auto sm:max-w-6xl">
        <DialogHeader>
          <DialogTitle>{t("手动连线校准尺寸")}</DialogTitle>
          <DialogDescription>{t("AI先找测量板四角和服装测量点，系统按 120 × 160 cm 板面校正透视并换算厘米。员工只需调整错误点位；AI原始点不会被覆盖。")}</DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-4">
            <div className={cn("space-y-2 rounded-md border p-3", calibratingBoard ? "border-amber-400 bg-amber-50" : "border-emerald-300 bg-emerald-50/60")}>
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-sm font-medium">{t("测量板透视校准")}</p>
                  <p className="mt-1 text-xs text-muted-foreground">
                    {calibratingBoard
                      ? t("请点击{v0}（{v1}/4）。", { v0: t(BOARD_CORNER_LABELS[calibrationPoints.length] ?? "四角"), v1: Math.min(calibrationPoints.length + 1, 4) })
                      : boardCalibrationSource === "AI"
                        ? t("AI 已找到四个板面校准点，请检查绿色边框；不准时点击重新校准。")
                        : boardCalibrationSource === "MANUAL"
                          ? t("已使用本次人工校准的四个板角建立坐标。")
                          : t("已按固定机位保存的板角建立坐标；不准时点击重新校准。")}
                  </p>
                </div>
                <Button type="button" size="sm" variant="outline" onClick={beginBoardCalibration}>
                  <ScanLineIcon data-icon="inline-start" />{boardCalibration ? t("重新校准") : t("校准板面")}
                </Button>
              </div>
              {calibratingBoard && calibrationPoints.length ? (
                <Button type="button" size="sm" variant="ghost" onClick={() => setCalibrationPoints((current) => current.slice(0, -1))}>
                  <Undo2Icon data-icon="inline-start" />{t("撤销上一个板角")}
                </Button>
              ) : null}
            </div>
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {activeLine?.source === "AI"
                      ? t("AI 已先定位起点和终点；不准时点击重画本项。")
                      : activeLine
                        ? t("已按人工点位和板面坐标换算，可重画或修正数值。")
                        : start ? t("已选起点，请点击终点。") : t("请先点击起点，再点击终点。")}
                  </p>
                  {activeMeasurement.key === "shoulderWidthCm" ? (
                    <p className="mt-1 text-xs font-medium text-amber-700">{t("从左侧肩袖接缝连接到右侧肩袖接缝；不要从领口开始。")}</p>
                  ) : null}
                </div>
                <label className="block space-y-1 text-sm">
                  <span>{t("测量板换算（cm）")}</span>
                  <Input inputMode="decimal" value={values[activeMeasurement.key] ?? ""} onChange={(event) => updateValue(event.target.value)} />
                </label>
                  {activeMeasurement.aiValue ? <p className="text-xs text-muted-foreground">{t("AI 原值：")}{activeMeasurement.aiValue} cm{activeLine?.source === "AI" ? t(" · AI点位") : ""}</p> : null}
                <div className="flex flex-wrap gap-2">
                  <Button type="button" size="sm" variant="outline" disabled={!start} onClick={() => setStart(null)}><Undo2Icon data-icon="inline-start" />{t("撤销起点")}</Button>
                  <Button type="button" size="sm" variant="outline" disabled={!activeLine && !start} onClick={redrawActive}><RotateCcwIcon data-icon="inline-start" />{t("重画本项")}</Button>
                </div>
              </div>
            ) : null}

            <div className="rounded-md bg-muted/50 p-3 text-xs text-muted-foreground">
              <p className="font-medium text-foreground"><MousePointer2Icon className="mr-1 inline size-3.5" />{t("连线规则")}</p>
              <p className="mt-1">{t("肩宽必须连接左右肩缝端点，不是领口到单侧肩；胸宽、腰宽连接两侧端点；衣长纵向连接起止点；袖长从肩缝连接到袖口。")}</p>
            </div>
        ...
        <DialogFooter>
          <Button type="button" variant="outline" onClick={() => props.onOpenChange(false)}>{t("取消")}</Button>
          <Button type="button" disabled={!boardCalibration || calibratingBoard} onClick={apply}><CheckIcon data-icon="inline-start" />{t("应用人工连线")}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
