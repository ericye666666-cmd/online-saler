...
export function ProductBatchBarcodePage({ batchId }: { batchId: string }) {
  ...
  async function confirmManualPrint() {
    ...
    try {
      ...
      setNotice("已确认本批标签完成打印。请按商品顺序贴码，完成后可直接进入商品审核。");
    } catch (caught) {
  ...
  const readyForReview = printedCount === batch.targetCount;
  ...
  return (
    <div className="flex min-w-0 flex-col gap-5">
      ...
      {readyForReview ? <StatusMessage tone="neutral"><span className="flex items-center gap-2"><CheckCircle2Icon className="size-4 text-emerald-600" />本批 {batch.targetCount} 张标签已打印。请按顺序贴码后进入审核。</span></StatusMessage> : null}
    </div>
  );
}
