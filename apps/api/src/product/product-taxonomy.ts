...
/** "CATEGORY: SUB, SUB; ..." for the AI prompt, so a subcategory is only picked under the category that owns it. */
export function subcategoryPairs(document: ProductTaxonomyDocument): string {
  const subcategories = document.groups.SUBCATEGORY.filter((option) => option.active);
  return activeTaxonomyCodes(document, "CATEGORY")
    .map((category) => {
      const owned = subcategories.filter((option) => option.parentCode === category).sort(sortOptions).map((option) => option.code);
      return `${category}: ${[...owned, "OTHER"].join(", ")}`;
    })
    .join("; ");
}
