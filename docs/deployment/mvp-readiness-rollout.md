...
|---|---|---|
| [deploy-storefront-staging.yml](../../.github/workflows/deploy-storefront-staging.yml) | Named “Deploy Live Storefront”; pushes to `develop` deploy `online-saler-storefront-staging` with `MPESA_ENV=production`, `MPESA_ENVIRONMENT=production`, production M-Pesa secrets (so it charges the full order total) and `DATABASE_URL=STAGING_DATABASE_URL:latest` | Do not treat this service or database as disposable or payment-isolated because its name contains staging |
